...
%If we add a note that the full guide is out of date, but the new things are mentioned here then I (Gordon) could more easily make some updates to the program to make it easier to use (e.g. context menu option to add required/optional docblocks to a system or to add a config block to a subsystem to use instead of an external file) without having to worry about a bunch of extra documentation in that file.
...
\begin{enumerate}
	...
	\item Install the \href{https://www.mathworks.com/matlabcentral/fileexchange/49897-signature-tool}{Signature Tool}.
